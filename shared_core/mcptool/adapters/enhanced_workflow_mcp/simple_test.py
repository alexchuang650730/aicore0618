...
async def test_workflow_generation():
    ...
    try:
        ...
        if result["status"] == "success":
            print(f"✅ 工作流生成成功: {result['workflow_id']}")
            print(f"   节点数量: {result['nodes_count']}")
            print(f"   边数量: {result['edges_count']}")
            return True
        else:
            print(f"❌ 工作流生成失败: {result['message']}")
            return False
...
async def test_state_management():
    ...
    try:
        ...
        # 获取工作流
        retrieved_workflow = await state_manager.get_workflow("test_workflow_state")
        if retrieved_workflow and retrieved_workflow.id == "test_workflow_state":
            print("✅ 工作流获取成功")
        else:
            print("❌ 工作流获取失败")
            return False
        ...
        # 更新状态
        result = await state_manager.update_workflow_status("test_workflow_state", "running")
        if result["status"] == "success":
            print("✅ 工作流状态更新成功")
            return True
        else:
            print(f"❌ 工作流状态更新失败: {result['message']}")
            return False
